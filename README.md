...
- **Validação de Datas em Query Parameters:** Para parâmetros de data como `data_inicio` e `data_fim`, a validação estrita `@IsISO8601()` foi comentada nos DTOs de query de analytics para melhor compatibilidade com o Swagger UI. A transformação para `Date` e a validação `@IsDate()` são mantidas.
- **Consultas de Analytics:** Utilizam o QueryBuilder do TypeORM. Funções específicas do banco (ex: `strftime` para SQLite para agrupamento diário) são usadas. A lógica comum de consulta para agregações diárias (MAX, AVG) foi refatorada em métodos privados no `AnalyticsService`. O cálculo de Geração utiliza uma função auxiliar fornecida, adaptada e integrada.
- **Testes Automatizados:** O projeto inclui:
    - **Testes Unitários** (Jest) para a lógica de negócios nos Services e para a lógica de delegação nos Controllers, utilizando mocks para isolamento.
    - **Testes End-to-End** (Jest + Supertest) para validar o fluxo completo de todos os endpoints da API, incluindo a interação com o banco de dados (SQLite) e a validação de DTOs.